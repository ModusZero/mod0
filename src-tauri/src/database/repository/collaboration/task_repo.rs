use super::task::{Task, TaskStatus, TddStatus};
...
impl<'a> Repository<'a> {
    ...
    pub async fn upsert_task(&self, task: &Task) -> sqlx::Result<()> {
        sqlx::query!(
    ...
    pub async fn get_session_tasks(&self, session_id: &str) -> sqlx::Result<Vec<Task>> {
        sqlx::query_as!(
            Task,
            r#"SELECT
                id as "id!",
