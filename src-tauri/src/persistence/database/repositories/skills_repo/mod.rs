...
impl<'a> Repository<'a> {
    ...
    pub async fn upsert_skill(&self, skill: &Skill) -> sqlx::Result<()> {
        sqlx::query!(
            r#"INSERT INTO skill (id, name, description, file_path, tags) 
               VALUES (?, ?, ?, ?, ?)
               ON CONFLICT(id) DO UPDATE SET 
               description = excluded.description,
               file_path = excluded.file_path,
               tags = excluded.tags"#,
            skill.id, skill.name, skill.description, skill.file_path, skill.tags
        )
    ...
    pub async fn get_skills_by_tags(&self, tags: Vec<String>) -> sqlx::Result<Vec<Skill>> {
        ...
        let mut query_builder: QueryBuilder<Sqlite> = QueryBuilder::new(
            "SELECT id, name, description, file_path, tags FROM skill WHERE "
        );
